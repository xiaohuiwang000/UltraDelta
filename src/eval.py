...
def eval_dataset(image_encoder, dataset_name, args):
    image_encoder.eval()

    dataset = get_dataset(dataset_name, preprocess_val, location=args.data_location,  batch_size=args.batch_size)
    dataloader = get_dataloader(dataset, is_train=False, args=args, image_encoder=None)
    ...
    metrics = {'top1': top1}
    print(f'Done evaluating on dataset. Accuracy: {100 * top1:.2f}%')
